...
bpv2 <- function(fcasts, nodes = NULL, groups = NULL, weights = NULL, alg, control.nn = list())
{
  ...
  z <- zfution(algint,weights,smat,fcasts,weiint)
  y <- yfution(algint,weights,smat,fcasts,weiint)
  
  #89-115求解布尔值converged
  tol <- sqrt(.Machine$double.eps)#精度
  tzb <- t(z) %*% as.matrix(b)
