...
#' }
#' @export combinef
combinef2 <- function(fcasts, nodes = NULL, groups = NULL, weights = NULL, nonnegative = FALSE,
  ...
  if (!nonnegative) {
    if (is.null(groups)) { # hts class
      ...
      if (alg == "recursive") { # only nodes to be needed
        ...
        if (alg == "chol") {
          ...
          if (alg == "lu") {
            allf <- LU2(fcasts = fcasts, S = smat, weights = weights, allow.changes = FALSE)
          } else if (alg == "cg") {
    ...
    if (parallel) {
      ...
      bf <- parSapplyLB(cl = cl, X = lst.fc, bpv2, nodes = nodes, groups = groups, weights = weights, alg = alg, control.nn = control.nn, simplify = TRUE)
      stopCluster(cl = cl)
    } else {
      bf <- sapply(lst.fc, bpv2, nodes = nodes, groups = groups, weights = weights, alg = alg, control.nn = control.nn)
    }
